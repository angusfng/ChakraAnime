...
import { Flex, Box, Image, Text, Heading, Divider, Center } from "@chakra-ui/react";

function InformationCardOne(props) {
  return (
    <Flex bg="white" boxShadow="lg" borderRadius="10px" flexDirection="column" p="2rem">
      <Heading as="h3" fontSize="1.5em">Information</Heading>
      <Divider my="1rem"/>
      <Flex>
        <Center>
          <Image maxW="30rem" objectFit="cover" src={props.json.image_url} alt="Information card image" pr="2rem"/>
        </Center>
        <Flex flexDirection="column" justifyContent="space-between">
          ...
          <Box>
            ...
            {props.genres.map((genre, idx) =>(
              <Text key={idx} d="inline">{(idx ? ", " : "") + genre.name}</Text>
            ))}
          </Box>
          <Box>
          ...
          <Box>
            ...
            {props.studios.map((studio, idx) =>(
              <Text key={idx} d="inline">{(idx ? ", " : "") + studio.name}</Text>
            ))}
          </Box>
          <Box>
